...
class Layout {
  ...
  createMatcher(keyword, noParam = false) {
    return new RegExp(this.leftDelimiter + '\\s*(' + keyword + ')' + (noParam ? '' : '(?:\\s+([^' + this.rightDelimiter + ']+?)|\\s*)') + '\\s*' + this.rightDelimiter
      + '([\\s\\S]*?)' + this.leftDelimiter + '\\s*/' + keyword + '\\s*' + this.rightDelimiter + '(?:\\r?\\n)?', 'g');
  }

  createOpenMatcher(keyword) {
    return new RegExp(this.leftDelimiter + '\\s*(' + keyword + ')(?:\\s+([^' + this.rightDelimiter + ']+?)|\\s*)\\s*' + this.rightDelimiter + '(?:\\r?\\n)?', 'g');
  }

  createPlainMatcher(keyword) {
    return new RegExp('(' + this.leftDelimiter + '\\s*(' + keyword + ')(?:\\s+([^' + this.rightDelimiter + ']+?)|\\s*)\\s*' + this.rightDelimiter
      + ')([\\s\\S]*?)(' + this.leftDelimiter + '\\s*/' + keyword + '\\s*' + this.rightDelimiter + '(?:\\r?\\n)?)', 'g');
  }
};
